<template>
  <main class="container cart__wrap">
    <section>
      <CartProductCard
        v-for="item of cart"
        v-bind:key="item.id"
        v-bind:data="item"
      >
        <button class="btn" v-on:click="remove(item.id)">
          <svg
            width="18"
            height="18"
            viewBox="0 0 18 18"
      ...
      <div class="cart__flex cart__control">
    ...
    <section>
      <form class="cart__form" action="#">
        <p>SHIPPING ADRESS</p>
        <input type="text" placeholder="Сountry" value="Bangladesh" />
